...
class CELoss(nn.Module):
    """Semantic Swgmentation, Human Parts Segmentation, Saliency Detection"""
...
def get_loss_functions(task_loss_config):
    """
    Get loss functions for each task
    :param tasks: Specified tasks
    :param losses_config: Loss functions defined in config
    :return: Dict of loss functions
    """
    key2loss = {
        "CELoss": CELoss,
...
def get_criterion(dataname, tasks):
    if dataname == 'pascalcontext':
        ...
        losses_config = CITYSCAPES_LOSS_CONFIG
    else:
