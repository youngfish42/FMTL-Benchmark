...
config_dict = dict(hrnetv2p_w18_small=dict(stage1=dict(num_modules=1,
                                                       num_branches=1,
                                                       block='BOTTLENECK',
                                                       num_blocks=(2, ),
                                                       num_channels=(64, )),
                   ...
                   hrnetv2p_w48=dict(stage1=dict(num_modules=1,
                                                 num_branches=1,
                                                 block='BOTTLENECK',
                                                 num_blocks=(4, ),
                                                 num_channels=(64, )),
...
class HRModule(nn.Module):
    ...
    def __init__(self,
        ...
        self.branches = self._make_branches(num_branches, blocks, num_blocks, num_channels, norm_layer)
    ...
    def _make_one_branch(self, branch_index, block, num_blocks, num_channels, norm_layer, stride=1):
        """Build one branch."""
        downsample = None
        if stride != 1 or \
            ...
            downsample = nn.Sequential(
                nn.Conv2d(self.in_channels[branch_index],
                          ...
                          kernel_size=1,
                          stride=stride,
                          bias=False), norm_layer(num_channels[branch_index] * block.expansion))
        ...
        for i in range(1, num_blocks[branch_index]):
            layers.append(block(self.in_channels[branch_index], num_channels[branch_index], norm_layer=norm_layer))
    ...
    def _make_branches(self, num_branches, block, num_blocks, num_channels, norm_layer):
        ...
        for i in range(num_branches):
            branches.append(self._make_one_branch(i, block, num_blocks, num_channels, norm_layer))
                            ...
                            conv_downsamples.append(
                                nn.Sequential(
                                    nn.Conv2d(in_channels[j],
                                              ...
                                              stride=2,
                                              padding=1,
                                              bias=False), norm_layer(in_channels[i])))
                        else:
                            conv_downsamples.append(
                                nn.Sequential(
                                    nn.Conv2d(in_channels[j],
                                              ...
                                              stride=2,
                                              padding=1,
                                              bias=False), norm_layer(in_channels[j]), nn.ReLU(inplace=False)))
            ...
            for j in range(self.num_branches):
                if i == j:
                    ...
                    y = y + nn.functional.interpolate(
                        self.fuse_layers[i][j](x[j]), size=x[i].shape[2:], mode='bilinear', align_corners=False)
                else:
...
class HRNet(nn.Module):
    ...
    def __init__(self, extra, in_channels=3, norm_layer=nn.BatchNorm2d, norm_eval=False, zero_init_residual=False):
        ...
        self.layer1 = self._make_layer(block, 64, num_channels, num_blocks, norm_layer)
        ...
        self.transition1 = self._make_transition_layer([stage1_out_channels], num_channels, norm_layer)
        self.stage2, pre_stage_channels = self._make_stage(self.stage2_cfg, num_channels, norm_layer)
        ...
        block = self.blocks_dict[block_type]
        num_channels = [channel * block.expansion for channel in num_channels]
        self.transition2 = self._make_transition_layer(pre_stage_channels, num_channels, norm_layer)
        self.stage3, pre_stage_channels = self._make_stage(self.stage3_cfg, num_channels, norm_layer)
        ...
        block = self.blocks_dict[block_type]
        num_channels = [channel * block.expansion for channel in num_channels]
        self.transition3 = self._make_transition_layer(pre_stage_channels, num_channels, norm_layer)
        self.stage4, pre_stage_channels = self._make_stage(self.stage4_cfg, num_channels, norm_layer)

    def _make_transition_layer(self, num_channels_pre_layer, num_channels_cur_layer, norm_layer):
                    ...
                    transition_layers.append(
                        nn.Sequential(
                            nn.Conv2d(num_channels_pre_layer[i],
                                      ...
                                      stride=1,
                                      padding=1,
                                      bias=False), norm_layer(num_channels_cur_layer[i]), nn.ReLU(inplace=True)))
                else:
...
def hrnetv2p_w18_s(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> HRNet:
    ...
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(model_urls['hrnetv2p_w18_small'], progress=progress)
        new_state_dict = {}
...
def hrnetv2p_w48(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> HRNet:
    ...
    model = HRNet(extra=extra, **kwargs)
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(model_urls['hrnetv2p_w48'], progress=progress)
        new_state_dict = {}
        for k, v in state_dict.items():
            # skip the blocks of the classification head
            if k.startswith('final_layer'):
