...
def get_transformations(size, train=True):
    ...
    :param tuple size: Output size
    :param bool train: Train or validation, defaults to True
...
class RandomScaling(object):
    ...
    :param float min_scale_factor: Minimum scale value
    :param float max_scale_factor: Maximum scale value
    """
...
class RandomCrop(object):
    """
    Randomly crop image if it exceeds desired size
    ...
    def __call__(self, sample):
        ...
        for key, target in sample.items():
            sample[key] = self.random_crop(key, target, crop_location)
        return sample


class RandomHorizontallyFlip(object):
    """
    Randomly horizontal flip image and labels with probability of 0.5
    """
