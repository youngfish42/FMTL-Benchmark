...
class PASCALContext(torch.utils.data.Dataset):
    ...
    def __init__(self, root=MyPath.db_root_dir('PASCALContext'), dataidxs=None, train=True, transform=None, tasks=None):
        """
        Initialize the PASCAL-Context Multi-Task Dataset
        :param root: Root directory of dataset
        :param train: True for training set, False for validation set
        :param input_size: Size of input data to model
        :param tasks: Tasks to be loaded
        :param val_num: Number of samples used in validation set
        """
        self.root = root
        ...
        if self.do_human_parts:
            ...
            if not self.do_edge and not self.do_semseg and not self.do_sal and not self.do_normals:
                ...
                assert (len(self.images) == len(self.parts))
                print('Number of images with human parts: {:d}'.format(len(self.parts)))
    ...
    def _load_semseg(self, index):
        _semseg = np.array(Image.open(self.semsegs[index]), dtype=np.float32, copy=False)
        return _semseg
    ...
    def _load_sal(self, index):
        _sal = np.array(Image.open(self.sals[index]), dtype=np.float32, copy=False) / 255.  # [0,255] => [0,1]
