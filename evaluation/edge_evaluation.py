...
def nms_process(save_dir, idx):
    ...
    :param save_dir: Path of edge prediction directory
    :return: None
    """
    cwd = os.getcwd()
    os.chdir(seism_root)
...
def display_edge_eval_result(save_dir, idx):
    """
    Display edge evaluation result and clean up files
    :param exp_name: Name of experiment
    :param save_dir: Path of edge prediction directory and evaluation result
    :return: None
    """
    # Collect results from txt file
    with open(os.path.join(save_dir, idx + '_edge_test.txt'), 'r') as f:
    ...
    # Print result
    print('Edge Detection odsF: {0:.4f}'.format(100 * eval_dict['odsF']))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--idxs', nargs='+', default=['0', '1', '2', '3'])
    parser.add_argument('--datasets', nargs='+', default=['NYUD', 'NYUD', 'NYUD', 'NYUD'])
    ...
    for i, idx in enumerate(args.idxs):
        if not args.done:
            ...
            display_edge_eval_result(save_dir, idx)
